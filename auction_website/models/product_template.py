...
# Actualizar cambios del modelo en db (solo si hay cambios): -u all -d remates6 
# Hacer cambios en views xml sin tener que instalar la app cada vez: --dev xml
# python3 odoo-bin  --config=/Users/marcelocaiafa/projects/2020/remates.odoo/env/odoo/debian/odoo.conf --dev xml -u all -d remates6
class ProductTemplate(models.Model):
    ...
    #Agregado @chelo. asignarle al lote numero esto es != al id.
    lot_number      = fields.Integer(string="Lot number")

    show_current_bid    = fields.Boolean(compute="get_current_bid_id",store=False)
    show_lot_end_date   = fields.Boolean(compute="get_lot_end_date",store=False)
    show_next_link_time = fields.Boolean(compute="get_next_link_time",store=False)

    def get_lot_number(self):
        if(len(str(self.lot_number)) == 1):
            return f"00{self.lot_number}"
        if(len(str(self.lot_number)) == 2):
            return f"00{self.lot_number}"
        return self.lot_number
    
    def get_current_bid_id(self):
        self.show_current_bid = True if self.current_bid_id.value != 0 else False 

    def get_lot_end_date(self):
        self.show_lot_end_date = True if self.lot_end_date else False
    
    def get_next_link_time(self):
        self.show_next_link_time = True if self.next_link_time else False
    ...
    def prepare_invoice_fields(self):
        lot_end_date = datetime.strptime(str(self.lot_end_date), '%Y-%m-%d %H:%M:%S').date()
        invoice_date_due = lot_end_date + timedelta(days=self.auction_id.time_between_payment_links)
        fields = {

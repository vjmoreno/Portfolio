...
class hr_employee(models.Model):
    ...
    def add_one_year_points(self):
                ...
                if contract.date_end:
                    ...
                    if date_end > date:
                        if contract.employee_id not in contracts_dict.keys():
                            ...
                            contracts_dict[contract.employee_id] = min(contracts_dict[contract.employee_id],
                                                                       contract.date_start)
        ...
        for employee, value in contracts_dict.items():
            ...
            if (contract_date.month == date.month) and (contract_date.day == date.day) and (
                    contract_date.year < date.year):
    ...
    def subtract_administrative_points(self):
        for employee in self.env['hr.employee'].search([]):
            ...
            for points in employee.points_ids:
                ...
                if (date.month == today.month) and (date.day == today.day) and (date.year + 1 == today.year) and (
                        points.behavior == 'Ano de contrato'):
                    discount = True
            if discount:

...
class Invoice(models.TransientModel):
	...
	def onchange_amount(self):
		if len(str(self.amount).split('.')) > 1:
			if len(str(self.amount).split('.')[1]) > 2:
				return {'value':{},'warning':{'title':'Atencion!','message':'El monto neto debe tener maximo 2 decimales'}}
	...
	def send_invoice(self):
		...
		period_id=period_obj.search([('date_start','<=',self.emission_date),('date_stop','>=',self.emission_date),('company_id','=',self.company_id.id),('special','=',False)])
